...
class MultiChannelExportPipelineSetup(Operator):
    ...
    def execute(self, context):
        ...
        # Function to set up VSE for compositing frames to video
        def setup_compositor(scene_name, input_dir, output_dir, is_mobile=True):
            ...
            # Only set detailed encoding settings if we're not using existing scene settings
            if not self.use_scene_settings:
            ...
            if frames:
                ...
                if self.loop_extend_frames:
                    # Forward + hold + reverse + hold
                    new_end_frame = (num_frames * 2) + (self.hold_frames * 2)
                ...
                if self.loop_extend_frames and num_frames > 1:
                    ...
                    last_frame_strip.frame_final_duration = self.hold_frames
                    
                    # 2. Add reversed sequence
                    # We need to create a placeholder and then fill it with the right frames in reverse
                    ...
                    # We need to access the strip elements to reverse them
                    ...
                    first_frame_strip = strips.new_image(
                        ...
                        filepath=first_frame,
                        channel=1,
                        frame_start=num_frames * 2 + self.hold_frames + 1
                    )
                    first_frame_strip.frame_final_duration = self.hold_frames
                
                self.report({'INFO'}, f"Added {num_frames} frames to {comp_scene_name}")
                
                # If looping, update info
                if self.loop_extend_frames:
                    self.report({'INFO'}, f"Created loop animation with {self.hold_frames} hold frames")
            else:
                ...
                # The text property expects a string, not a Text datablock
...
classes = (
    ...
    SwitchToSceneOperator, # Add the new scene switcher
